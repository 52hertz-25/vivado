...
void append_weight(const std::string &root, const std::string &layer,
                   ...
                   std::vector<lenet_w8a8::weight_t> *weights) {
    const std::vector<int> values =
        load_ints(join_path(root, layer + ".weight.int8.txt"), count);
    for (std::size_t i = 0; i < count; ++i) {
        if (values[i] < -127 || values[i] > 127) {
            throw std::runtime_error("INT8 range violation in " + layer);
        }
        (*weights)[offset + i] = values[i];
    }
}

std::vector<lenet_w8a8::weight_t> load_all_weights(const std::string &root) {
    using namespace lenet_w8a8;
...
lenet_w8a8::act_t quantize_pixel(unsigned char pixel) {
    const double mean = 0.1307;
    const double stddev = 0.3081;
    const double input_scale = 0.02221643079922894;
    ...
    value = std::max(-127L, std::min(127L, value));
...
int main(int argc, char **argv) {
    try {
        if (argc < 5) {
            std::cerr << "Usage: tb_lenet_w8a8_mnist <dataset_root> "
        ...
        std::cout << "W8A8_MNIST_BEGIN samples=" << limit << "\n";
        for (int sample = 0; sample < limit; ++sample) {
            ...
            lenet_w8a8_top(input.data(), weights.data(), logits.data());
            int predicted = 0;
            ...
            if (predicted == expected) {
                ++correct;
            } else if (sample < 100 || (limit - correct) <= 50) {
                std::cout << "MISCLASSIFIED index=" << sample
                          << " expected=" << expected
                          << " predicted=" << predicted << "\n";
            }
        ...
        std::cout << "W8A8_MNIST_SUMMARY samples=" << limit
                  << " correct=" << correct
                  << " accuracy=" << std::fixed << std::setprecision(4)
                  << accuracy << "% threshold=90.0000% ["
                  << (threshold_pass ? "PASS" : "FAIL") << "]\n";
        std::cout << "W8A8_MNIST_RESULT_FILE=" << result_csv << "\n";
        return threshold_pass ? 0 : 1;
    } catch (const std::exception &error) {
        std::cerr << "W8A8_MNIST_ERROR: " << error.what() << "\n";
        return 2;
    }
}
