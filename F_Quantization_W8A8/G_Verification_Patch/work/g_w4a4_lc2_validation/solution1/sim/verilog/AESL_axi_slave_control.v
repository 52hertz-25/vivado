...
//------------------------Parameter----------------------
`define TV_IN_feature_in "../tv/cdatafile/c.lenet_w4a4_top.autotvin_feature_in.dat"
`define TV_IN_weight "../tv/cdatafile/c.lenet_w4a4_top.autotvin_weight.dat"
`define TV_IN_logits "../tv/cdatafile/c.lenet_w4a4_top.autotvin_logits.dat"
...
parameter weight_c_bitwidth = 64;
parameter logits_DEPTH = 1;
reg [31 : 0] logits_OPERATE_DEPTH = 0;
parameter logits_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter lenet_w4a4_top_continue_addr = 0;
parameter lenet_w4a4_top_auto_start_addr = 0;
parameter feature_in_data_in_addr = 16;
parameter weight_data_in_addr = 28;
parameter logits_data_in_addr = 40;
...
reg  BREADY_reg = 0;
reg [feature_in_c_bitwidth - 1 : 0] mem_feature_in [feature_in_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_feature_in [ (feature_in_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * feature_in_DEPTH -1 : 0] = '{default : 'hz};
reg feature_in_write_data_finish;
reg [weight_c_bitwidth - 1 : 0] mem_weight [weight_DEPTH - 1 : 0] = '{default : 'h0};
...
reg [logits_c_bitwidth - 1 : 0] mem_logits [logits_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_logits [ (logits_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * logits_DEPTH -1 : 0] = '{default : 'hz};
reg logits_write_data_finish;
...
//write logits reg
reg [31 : 0] write_logits_count = 0;
reg write_logits_run_flag = 0;
reg write_one_logits_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & feature_in_write_data_finish & weight_write_data_finish & logits_write_data_finish;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_feature_in_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_feature_in[write_feature_in_count * four_byte_num  + i]!==feature_in_data_tmp_reg) begin
                    write (feature_in_data_in_addr + write_feature_in_count * four_byte_num * 4 + i * 4, feature_in_data_tmp_reg, write_feature_in_resp);
                    image_mem_feature_in[write_feature_in_count * four_byte_num + i]=feature_in_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        logits_write_data_finish <= 0;
        write_logits_run_flag <= 0; 
        write_logits_count = 0;
        count_operate_depth_by_bitwidth_and_depth (logits_c_bitwidth, logits_DEPTH, logits_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            logits_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_logits_run_flag <= 1; 
            write_logits_count = 0;
        end
        if (write_one_logits_data_done === 1) begin
            write_logits_count = write_logits_count + 1;
            if (write_logits_count == logits_OPERATE_DEPTH) begin
                write_logits_run_flag <= 0; 
                logits_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_logits
    integer write_logits_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] logits_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = logits_c_bitwidth;
    process_num = 3;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_3_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_logits_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write logits data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (logits_c_bitwidth < 32) begin
                        logits_data_tmp_reg = mem_logits[write_logits_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < logits_c_bitwidth) begin
                                logits_data_tmp_reg[j] = mem_logits[write_logits_count][i*32 + j];
                            end
                            else begin
                                logits_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_logits[write_logits_count * four_byte_num  + i]!==logits_data_tmp_reg) begin
                    write (logits_data_in_addr + write_logits_count * four_byte_num * 4 + i * 4, logits_data_tmp_reg, write_logits_resp);
                    image_mem_logits[write_logits_count * four_byte_num + i]=logits_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_logits_data_done <= 1;
                @(posedge clk);
                write_one_logits_data_done <= 0;
            end   
            process_3_finish <= 1;
        end
        @(posedge clk);
      ...
      for(i = 0; i < feature_in_DEPTH; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_feature_in [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_logits_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [logits_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (logits_c_bitwidth , factor);
  fp = $fopen(`TV_IN_logits ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_logits); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < logits_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_logits [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
