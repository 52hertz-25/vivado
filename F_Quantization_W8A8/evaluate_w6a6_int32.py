...
WEIGHT_DIR = BASE_DIR / "exported_weights" / "int6"
...
def get_scale(data, name):
    ...
    if isinstance(item, dict):
        for key in ("scale", "scale_int6", "scale_value"):
            if key in item:
...
def load_int6_weight(name):
    path = WEIGHT_DIR / f"{name}.weight.int6.npy"

    if not path.exists():
        raise FileNotFoundError(
            f"找不到{name}的INT6权重文件：{path}"
        )
...
activation_scales = {
    name: float(activation_scale_data[name]["max_abs"]) / 31.0
    for name in (
        "input",
        "conv1_relu",
        "pool1",
...
with torch.no_grad():
    for batch_index, (images, labels) in enumerate(test_loader):
        ...
        int6_logits, current_max = w6a6_forward(images)

        fp32_predictions = fp32_logits.argmax(dim=1)
        int6_predictions = int6_logits.argmax(dim=1)
        ...
        int6_correct += (
            int6_predictions == labels
        ).sum().item()
        agreement += (
            fp32_predictions == int6_predictions
        ).sum().item()
...
int6_accuracy = 100.0 * int6_correct / total
agreement_rate = 100.0 * agreement / total
accuracy_drop = fp32_accuracy - int6_accuracy


print("=" * 60)
print("W6A6-INT32测试结果")
print("=" * 60)
print(f"测试图片数量：       {total}")
print(f"FP32正确数量：       {fp32_correct}")
print(f"W6A6正确数量：       {int6_correct}")
print(f"FP32准确率：         {fp32_accuracy:.2f}%")
print(f"W6A6-INT32准确率：   {int6_accuracy:.2f}%")
...
result = {
    ...
    "quantized_accuracy_percent": int6_accuracy,
...
json_path = BASE_DIR / "w6a6_int32_results.json"
...
csv_path = BASE_DIR / "w6a6_accuracy_results.csv"
...
existing_rows.append({
    ...
    "Activation_bits": 6,
    "Accumulator_bits": 32,
    "Accuracy_percent": f"{int6_accuracy:.2f}",
