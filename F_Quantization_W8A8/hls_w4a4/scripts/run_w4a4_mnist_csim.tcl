# Vitis HLS 2022.2: W4A4 full MNIST C-simulation only.
if {$argc < 5} {
...
set source_file [file join $root src lenet_w4a4.cpp]
set header_dir [file join $root include]
set tb_file [file join $root tb tb_lenet_w4a4_mnist.cpp]
...
open_project -reset w4a4_mnist_hls
...
puts "W4A4_MNIST_CSIM_START limit=$limit"
csim_design -argv [list $dataset_root $weight_root $limit $result_csv]
close_project
puts {W4A4_MNIST_CSIM_PASS}
exit 0
