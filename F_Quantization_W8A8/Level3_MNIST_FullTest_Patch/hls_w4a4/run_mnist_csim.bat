...
set "WEIGHT_ROOT=%PROJECT_ROOT%\model\int4_weights"
set "RESULT_DIR=%PROJECT_ROOT%\reports\mnist"
set "RESULT_CSV=%RESULT_DIR%\w4a4_mnist_%LIMIT%_results.csv"
...
echo Running W4A4 MNIST C-simulation, samples=%LIMIT%
vitis_hls -f "%PROJECT_ROOT%\scripts\run_w4a4_mnist_csim.tcl" "%PROJECT_ROOT%" "%DATASET_ROOT%" "%WEIGHT_ROOT%" "%LIMIT%" "%RESULT_CSV%"
set "RETURN_CODE=%ERRORLEVEL%"
if not "%RETURN_CODE%"=="0" (
    echo W4A4_MNIST_CSIM_BATCH_FAIL exit_code=%RETURN_CODE%
    exit /b %RETURN_CODE%
)
echo W4A4_MNIST_CSIM_BATCH_PASS
