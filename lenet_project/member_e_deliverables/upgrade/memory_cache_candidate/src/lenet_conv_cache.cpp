        ...
        OUTPUT_COL:
            ...
            INPUT_CHANNEL:
                for (int input_channel = 0; input_channel < IN_C;
                    ...
                    accumulator =
                        mac_5x5_pe(window, &weight[kernel_base], accumulator);
                }
...
#pragma HLS INTERFACE ap_ctrl_hs port=return
    ...
    // Cache Conv2 weights once per invocation so the compute loop reuses
    // local memory instead of repeatedly issuing AXI reads for every output.
    static lenet::weight_t weight_cache[lenet::CONV2_WEIGHT_ELEMS];
#pragma HLS BIND_STORAGE variable=weight_cache type=ram_2p impl=bram
#pragma HLS ARRAY_PARTITION variable=weight_cache cyclic factor=5 dim=1

CACHE_CONV2_WEIGHTS:
    for (int index = 0; index < lenet::CONV2_WEIGHT_ELEMS; ++index) {
#pragma HLS PIPELINE II=1
        weight_cache[index] = weight[index];
    }
    ...
    lenet::conv5x5_valid<
        ...
        lenet::POOL1_OUT_H,
        lenet::POOL1_OUT_W,
        lenet::CONV2_OUT_C>(feature_in, weight_cache, bias, feature_out);
}
