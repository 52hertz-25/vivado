...
// D2 experiment: split the 25 products into five independent partial sums.
// Each lane handles indices p, p+5, p+10, p+15 and p+20.
// Keeping this as a separate function makes it easy to switch back to mac_5x5.
data_t mac_5x5_pe5(
    const data_t window[KERNEL_ELEMS],
    const weight_t kernel[KERNEL_ELEMS],
    data_t accumulator) {
...
PE_GROUP:
    for (int group = 0; group < KERNEL_ELEMS / 5; ++group) {
#pragma HLS PIPELINE II=1
    PE_LANE:
        for (int pe = 0; pe < 5; ++pe) {
#pragma HLS UNROLL
            const int index = group * 5 + pe;
            partial[pe] += window[index] * kernel[index];
        }
    }
        ...
        OUTPUT_COL:
            ...
            INPUT_CHANNEL:
                for (int input_channel = 0; input_channel < IN_C;
                    ...
                    accumulator =
                        mac_5x5_pe5(window, &weight[kernel_base], accumulator);
                }
