...
std::vector<lenet_w8a8::act_t> load_quantized_input(const std::string &path) {
    const double mean = 0.1307;
    const double stddev = 0.3081;
    const double input_scale = 0.02221643079922894;
    const std::vector<float> raw =
        load_floats(path, lenet_w8a8::INPUT_ELEMS);
    std::vector<lenet_w8a8::act_t> result(raw.size());
...
void append_weight(
    ...
    std::size_t count,
    std::size_t offset,
    std::vector<lenet_w8a8::weight_t> *all_weight) {
    const std::vector<int> values = load_ints(
        join_path(weight_root, name + ".weight.int8.txt"), count);
    for (std::size_t i = 0; i < values.size(); ++i) {
        if (values[i] < -127 || values[i] > 127) {
            throw std::runtime_error("INT8 range violation in " + name);
        }
        (*all_weight)[offset + i] = values[i];
    }
}

std::vector<lenet_w8a8::weight_t> load_all_weights(
    const std::string &weight_root) {
...
int run_sample(
    ...
    const double output_scale = 0.0004378844303305191;
...
int main(int argc, char **argv) {
    try {
        if (argc < 3) {
            std::cerr << "Usage: tb_lenet_w8a8 <model_root> <int8_weight_root> [--multi]\n";
            return 2;
        }
